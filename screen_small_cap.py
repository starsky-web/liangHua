...
def get_all_stocks_total_share(stock_codes: List[str], year: int, quarter: int) -> Dict[str, float]:
    """
    获取指定股票列表的总股本
    ...
    for code in tqdm(stock_codes, desc="获取总股本", leave=False):
        try:
            rs = bs.query_profit_data(code=code, year=year, quarter=quarter)
            if rs.error_code == "0" and rs.next():
                row = rs.get_row_data()
                # totalShare 在最后一个字段
                total_share = float(row[-1]) if row[-1] else None
                if total_share and total_share > 0:
                    result[code] = total_share
        except Exception:
            pass
...
def get_stock_total_share(code: str, year: int, quarter: int) -> Optional[float]:
    """
    获取单只股票的总股本
...
def main():
    ...
    try:
        ...
        # 计算总股本查询所需的年份和季度
        trade_date_obj = datetime.strptime(trade_day, "%Y-%m-%d")
        year = trade_date_obj.year
        quarter = (trade_date_obj.month - 1) // 3 + 1
        if quarter == 0:
            year -= 1
            quarter = 4
        ...
        for idx, row in tqdm(stock_list.iterrows(), total=len(stock_list), desc="处理进度"):
            ...
            try:
                ...
                # 市值过滤：市值 = 股价 × 总股本
                total_share = get_stock_total_share(code, year, quarter)
                if total_share is not None:
                    # 市值（亿元）= 收盘价 × 总股本（股） / 100000000
                    market_cap_yi = ind["close"] * total_share / 100000000
        ...
        print(f"市值筛选范围: {args.market_cap_min}亿 ~ {args.market_cap_max}亿")
        ...
        # 保存CSV
        output_file = f"screen_small_cap_result_{trade_day}.csv"
